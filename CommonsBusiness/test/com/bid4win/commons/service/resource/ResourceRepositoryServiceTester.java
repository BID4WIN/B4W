...
public abstract class ResourceRepositoryServiceTester<STORAGE extends ResourceStorage<STORAGE, TYPE, USAGE>,
       ...
       extends ResourceServiceTester<STORAGE, TYPE, SESSION, ACCOUNT, GENERATOR>
{
  /**
  ...
  public void testCreateUsage_String_String_long() throws Exception
  {
    ...
    STORAGE storage = this.createStartingResource("", "test", this.getType1(), this.getFilename1());
    ...
    assertEquals("Wrong usage number", 1, storage.getUsageList().size());
    ...
    assertEquals("Wrong usage number", 2, storage.getUsageList().size());
    assertTrue("Wrong result", storage.identical(result.getStorage()));
  ...
  public void test_update_of_storage_and_usage() throws Exception
  {
    ...
    STORAGE storage1 = this.createStartingResource("", "test", this.getType1(), file1.getName());
    ...
    assertEquals("Wrong usage number", 2, storage.getUsageList().size());
    assertTrue("Wrong result", storage.same(storage2, new Bid4WinList<Bid4WinRelationNode>()));
    for(USAGE usage3 : storage.getUsageList())
    {
  ...
  public void testDeleteUsage_long() throws Exception
  {
    ...
    assertEquals("Wrong usage number", 1, result.getUsageList().size());
    assertTrue("Wrong result", storage.same(result, new Bid4WinList<Bid4WinRelationNode>()));
    assertTrue("Wrong result", result.getUsageList().getFirst().identical(usage2, new Bid4WinList<Bid4WinRelationNode>()));
    ...
    assertEquals("Wrong version", storage.getVersion() + 1, result.getVersion());
    assertEquals("Wrong usage number", 0, result.getUsageList().size());
    assertTrue("Wrong result", storage.same(result, new Bid4WinList<Bid4WinRelationNode>()));
  }
